...
def generate_excel_report_beneficiary(report_data):
    # إنشاء كائن Excel
    ...
    dispatches = report_data['Dispatch']
    
    # إضافة البيانات إلى Excel
    row = 4
    for dispatch in dispatches:
        ws.cell(row=row, column=1, value=dispatch.dispatch_date)
        ws.cell(row=row, column=2, value=dispatch.document_number)
        ws.cell(row=row, column=3, value=dispatch.beneficiary.full_name)
        ws.cell(row=row, column=4, value=dispatch.quantity_dispatched)
        ws.cell(row=row, column=5, value=dispatch.deliverer_name)
        ws.cell(row=row, column=6, value=dispatch.recipient_name)
        row += 1
...
def generate_excel_report_supplier(report_data):
    ...
    for receiving in receivings:
        ...
        ws.cell(row=row, column=5, value=receiving.deliverer_name)
